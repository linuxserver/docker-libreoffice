pipeline {
  ...
  environment {
    ...
    BUILD_VERSION_ARG = 'LIBREOFFICE_VERSION'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-libreoffice'
    CONTAINER_NAME = 'libreoffice'
    DOCKERHUB_IMAGE = 'linuxserver/libreoffice'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/libreoffice") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/libreoffice",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-libreoffice",\
                       "backendUrl":"https://ghcr.io/linuxserver/libreoffice",\
                       "publicUrl":"https://lscr.io/linuxserver/libreoffice"}' || :
              else
